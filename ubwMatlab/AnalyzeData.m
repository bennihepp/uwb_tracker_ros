...
for iterations=1:1
    ...
    for i=1:length(log_data_seq.Range)
        ...
        if ((~isempty(Range)) & (Range ~= -1) & (Range < 1e2))
            R_WB_tracker=RotFromQuatJ(log_data_seq.Tracker_q(:,i));
            [r1 r2 r3] = dcm2angle(R_WB_tracker);
            Yaw_Tracker(i) = r1;
            R_WB_target=RotFromQuatJ(log_data_seq.Target_q(:,i));
            [r1 r2 r3] = dcm2angle(R_WB_target);
            Yaw_Target(i) = r1;
